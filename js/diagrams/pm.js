...
let pmPolicy = 'none';       // Price control: 'none' | 'floor' | 'ceiling'

// Switch the price-control policy, highlight the active button, and redraw.
function pmSetPolicy(m) {
  pmPolicy = m;
...
function pmDraw() {
  ...
  document.getElementById('pmChart').innerHTML = buildPMSVGInner({
    ...
    isAnimating:  false,
    hideGrid: document.getElementById('pmHideGrid').checked,
    hideNums: document.getElementById('pmHideNums').checked,
    policy:   pmPolicy
  });
...
function buildPMSVGInner(cfg) {
  const { W, H, pad, title, yLbl, xLbl, vU, hU, dCol, sCol,
          ...
          showBracket = false, isAnimating = false,
          hideGrid = false, hideNums = false, policy = 'none' } = cfg;
  ...
  if (!showDiseq) {
    // ── At equilibrium (or a non-binding control) — standard eq dot + dashes ──
    ...
    // A non-binding price control: draw the control line where it sits, greyed out.
...
function pmReset() {
  ...
  pmSetPolicy('none');   // resets buttons + redraws
}
