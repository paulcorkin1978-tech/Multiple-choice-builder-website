...
function surEff(q){var mode=q.mode||'eq';function lossOK(k){if(k==='LOSSCS')return mode==='tax'||mode==='qty'||(mode==='price'&&q.param>=50);if(k==='LOSSPS')return mode==='tax'||(mode==='price'&&q.param<50);return true;}var out=[];(q.reveals||[]).forEach(function(k){if(k==='DWL'&&mode==='eq')return;if(k==='TAX'&&mode!=='tax')return;if((k==='LOSSCS'||k==='LOSSPS')&&!lossOK(k))return;out.push(k);});return out;}
...
function replayAnim(){
  ...
  else if(q.type==='tax'){curPos[cur]={dA:q.startDS||0,sA:q.ansSS||0,fpA:0};const rb2=document.getElementById('revealBtn');if(rb2)rb2.style.display=attempted(cur)?'':'none';}
