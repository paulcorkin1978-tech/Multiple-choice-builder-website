...
function mkSVG(q,dA,sA,fpA,isAnimating,shiftDirD=0,shiftDirS=0,animT=0,showStaticBracket=false){
  ...
  if(q.type==='ppf'){
    ...
    const txNeg=q.taxType==='negext',txPosE=q.taxType==='posext',txExt=txNeg||txPosE;
    ...
    for(let i=1;i<=GRID;i++){
      const txNP=txShowLbls&&(Math.abs(i-txPc)<0.4||(!txExt&&Math.abs(i-txPs)<0.4)||Math.abs(i-txP0)<0.4);
    ...
    if(txExt&&txOnGrid&&txM0){
      ...
      const txSocAtQm=txNeg?sPf(txQ0,sA,txSsc):dPf(txQ0,dA,txDm,txDsc);
      // Value of the unshifted (private) curve at the optimal quantity.
      const txPrivAtQs=txNeg?sPf(txQn,txStartSS,txSsc):dPf(txQn,txStartDS,txDm,txDsc);
      ...
      if(txShown.has('extwedge')){
        const pv=q=>txNeg?sPf(q,txStartSS,txSsc):dPf(q,txStartDS,txDm,txDsc);
        const sc=q=>txNeg?sPf(q,sA,txSsc):dPf(q,dA,txDm,txDsc);
...
function showQ(i){
  ...
  if(rb) rb.style.display=(q.type==='plain'||q.type==='table'||q.type==='ped'||q.type==='sur')?'none':'';
...
function replayAnim(){
  const q=qs[cur];
  if(q.type==='plain'||q.type==='table'||q.type==='ped'||q.type==='sur')return;
